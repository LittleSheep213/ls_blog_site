...
class LoginForm(forms.Form):
	...
	# required=True表示必须要填写，默认就是True，写出来方便学习
...
class RegForm(forms.Form):
	username = forms.CharField(label='username', required=True, max_length=30, min_length=3,
								widget=forms.TextInput(attrs={'class':'form-control', 
															  'placeholder':'please input username(3~30 characters)'})) 
	email = forms.EmailField(label='email', 
								widget=forms.EmailInput(attrs={'class':'form-control', 
															   'placeholder':'please input email address'})) 
	password = forms.CharField(label='password', min_length=6,
								widget=forms.PasswordInput(attrs={'class':'form-control', 
															  'placeholder':'please input password(more than 6 characters)'}))  
	password_again = forms.CharField(label='repeat password', min_length=6,
								widget=forms.PasswordInput(attrs={'class':'form-control', 
